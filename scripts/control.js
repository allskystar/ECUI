...
//{if $phase == "define"}//
    ...
    var UI_CONTROL = ui.Control = inheritsControl(function (el, options) {
...
//{else}//
    ...
    /**
     * 为控件添加/移除一个扩展样式。
     * 扩展样式分别附加在类型样式与当前样式之后(参见 getType 与 getPrimary 方法)，使用-号进行分隔。如果类型样式为 ec-control，当前样式为 demo，扩展样式 hover 后，控件主元素将存在四个样式，分别为 ec-control、demo、ec-control-hover 与 demo-hover。
     * @public
     *
